...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated Illusory loader file *)


Unprotect[Illusory`Private`Package`$TopLevelLoad];
Illusory`Private`Package`$TopLevelLoad =
	MemberQ[$ContextPath, "Global`"];


BeginPackage["Illusory`"];
...
$PackageContexts = {
	"Illusory`",
	"Illusory`Private`Package`",
	"Illusory`Private`Hidden`"
};
$PackageDeclared =
...
PackageFileContext[f_String?DirectoryQ] :=
	With[{s = PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s, Illusory], "`"]
	];
...
PackageScopeBlock[e_, scope_String : "Hidden"] /; TrueQ[$AllowPackageRescoping] :=
	With[{newcont = "Illusory`Private`" <> StringTrim[scope, "`"] <> "`"},
...
PackageDecontext[
	...
	With[{
		...
		ctx = "Illusory`Private`" <> StringTrim[scope, "`"] <> "`"
	},
		Replace[names,
...
If[(Clear@Illusory`Private`Package`$loadAbort;!#)&@Illusory`Private`Package`$loadAbort,
	Illusory`Private`Package`PackagePostProcessContextPathReassign[]
]