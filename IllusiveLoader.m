...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated Illusive loader file *)


Unprotect[Illusive`Private`Package`$TopLevelLoad];
Illusive`Private`Package`$TopLevelLoad=
	MemberQ[$ContextPath, "Global`"];


BeginPackage["Illusive`"];
...
$PackageContexts={
		"Illusive`",
		"Illusive`Private`Package`",
		"Illusive`Private`Hidden`"
		};
$PackageDeclared=
...
PackageFileContext[f_String?DirectoryQ]:=
	With[{s=PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s,Illusive],"`"]
		];
...
PackageScopeBlock[e_,scope_String:"Hidden"]/;TrueQ[$AllowPackageRescoping]:=
	With[{newcont="Illusive`Private`"<>StringTrim[scope,"`"]<>"`"},
...
PackageDecontext[
	...
	With[{
		...
		ctx="Illusive`Private`"<>StringTrim[scope,"`"]<>"`"
		},
		Replace[names,
...
If[(Clear@Illusive`Private`Package`$loadAbort;!#)&@Illusive`Private`Package`$loadAbort,
	Illusive`Private`Package`PackagePostProcessContextPathReassign[]
	]